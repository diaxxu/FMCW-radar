...
also this one is really good ![alt text](image-2.png) if any one is doing the same exact project i recommend that he watch those two series !! 
...
starting by the S-Band VCO Block its main job is to generates the high-frequency $2.4\text{ GHz}$ RF signal by converting the incoming $V_{\text{TUNE}}$ voltage ramp into a sweeping microwave frequency output ![alt text](image-3.png)
...
# 01/08/2026 
# *time spent: 4.41h*
alright i have some badnew about the latest phase we did after watching some vid on high frequency routing we did every mistake that he said a beguinner would do ![alt text](image-23.png)
...
ou shii ![alt text](image-45.png) since im using matlab free version i only have 20 hours per month :scared_skull: 
